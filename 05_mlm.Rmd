...
tab_model(
  nest_model_1a,
  show.se = TRUE, show.std = TRUE, show.stat = FALSE, p.val = "kr", show.df = FALSE, p.style = "stars",
  file = "05_Output/MLM_table_two.doc"
)
...
# There might be a case where you think that clinics  that are allocated participants that report worse symptoms at treatment start have better outcomes
# For this situation, you would want to allow for separate covariances in each treatment group
# To do this, update the variance-covariance matrix at the clinic level
treatment_model_5 <- lmer(
  si ~ time * treatment_arm +

    (1 | clinic:id) + # random intercept for each subject nested within clinic
    # ... allows each participant to have their own baseline level

    (1 | clinic) + # random intercept for each clinic
    # accounts for between-clinic differences in overall outcome levels

    (0 + treatment_arm + time:treatment_arm | clinic), # random slopes for treatment_arm and time × treatment_arm interaction by clinic
  #  This lets the effect of treatment vary by clinic
  # The '0' here is so that there isn't a clinic-level intercept - since that's already in (1 | clinic)
  data = dataset_nest
)

summary(treatment_model_5)
```
...
# Finally, in the case where you think that clinics  that are allocated participants that report worse symptoms at treatment start have better outcomes (more room for improvement):
conditional_growth_model_5 <- lmer(
  si ~ time * treatment_arm
    + (time | clinic:id)
    + (time | clinic)
    + (0 + treatment_arm + time:treatment_arm | clinic),
  data = dataset_nest
)
```
