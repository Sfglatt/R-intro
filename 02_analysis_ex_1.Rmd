...
if (!require("EGAnet")) {install.packages("EGAnet"); require("EGAnet")} # EGAnet is used for exploratory graph analysis (EGA)
...
# Data and directory
```{r data}
# Import the (toy) data that was saved from the 01_base_data script.
data <- read.csv("01_Output/Scored_data.csv")

# Filter the data to just the variables that are needed for analyses - all of the BDI-II items. 
filtered_data <- dplyr::select(data, starts_with("bdi_"))
...
ega_object <- EGA(data = filtered_data, 
                  corr = "auto", 
                  model = "glasso", 
                  algorithm = "walktrap",
                  uni.method = "louvain",
...
pdf("02_Output/EGA_plot.pdf", width = 10, height = 10) 
...
pdf("02_output/EGA_item_stability.pdf", width = 10, height = 10)
dimensionStability(boot_ega_object)
dev.off()

# To save the figures as a PNG file, 

png("02_Output/EGA_plot.png", width = 1000, height = 1000) 
...
png("02_Output/EGA_item_stability.png", width = 1000, height = 1000)
dimensionStability(boot_ega_object)
dev.off()
```
